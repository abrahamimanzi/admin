                ...
                <div class="box">
                    <div class="box-header with-border">
                        <?php
                            ...
                            $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `pass_type`='Visitor' || `pass_type` = 'Silver' || `pass_type` = 'Gold' || `pass_type` = 'Platinum' || `pass_type` = 'Speakers' || `pass_type` = 'Delegate' || `pass_type` = 'VVIP' || `pass_type` = 'VIP' AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
								$participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `pass_type`='Visitor' || `pass_type` = 'Silver' || `pass_type` = 'Gold' || `pass_type` = 'Platinum' || `pass_type` = 'Speakers' || `pass_type` = 'Delegate' || `pass_type` = 'VVIP' || `pass_type` = 'VIP' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                              ...
                              $participantTable = new Participant();
                              $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `pass_type`='Visitor' || `pass_type` = 'Silver' || `pass_type` = 'Gold' || `pass_type` = 'Platinum' || `pass_type` = 'Speakers' || `pass_type` = 'Delegate' || `pass_type` = 'VVIP' || `pass_type` = 'VIP' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
                                            ...
                                            <div id="participant-menu-content-<?=$participant_data->ID?>" class="hidden">
                                                <form method="post">
                                                    ...
                                                    <ul class="popover-menu-list">
                                                        ...
                                                        <?php
                                                        if(($session_user_data->groups == 'Admin' || $session_user_data->groups == 'TAS-Admin' || $session_user_data->groups == 'TAS-SUPER-Admin' || $session_user_data->groups == 'OGS-User') && ($participant_data->pass_type == 'Silver' ||$participant_data->pass_type == 'Gold' ||$participant_data->pass_type == 'Platinum' || $participant_data->pass_type == 'Exhibitor')){?>
                                                            <?php if($participant_data->payment_state=='Confirmed'){?>
                                                            <li><a class="menu" href="<?=DN?>/receipt-single/<?=$participant_data->token?>" target="invoice_tab"><i class="fa fa-print icon"></i> Print Invoice</a></li>
                                                            <?php }?>
