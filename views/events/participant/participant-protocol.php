              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='protocal' || `category`='Protocal' || `pass_type`='Protocol' || `category`='Protocol' AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `pass_type`='Protocol' || `category`='Protocol' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                              ...
                              $participantTable = new Participant();
                              $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `pass_type`='Protocol' || `category`='Protocol' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
