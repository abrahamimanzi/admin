              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE  (`pass_type`='Silver' || `pass_type`='Gold' || `pass_type`='Platinum' || `pass_type`='Delegate') AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT* FROM `events_participant` WHERE (`pass_type`='Silver' || `pass_type`='Gold' || `pass_type`='Platinum' || `pass_type`='Delegate') AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                                ...
                                $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE (`pass_type`='Silver' || `pass_type`='Gold' || `pass_type`='Platinum' || `pass_type`='Delegate') AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
                                            ...
                                            <div id="participant-menu-content-<?=$participant_data->ID?>" class="hidden">
                                                <form method="post">
                                                    ...
                                                    <ul class="popover-menu-list">
                                                        ...
                                                        <?php
                                                        if(($session_user_data->groups == 'Admin' || $session_user_data->groups == 'TAS-Admin' || $session_user_data->groups == 'TAS-SUPER-Admin' || $session_user_data->groups == 'OGS-User') && ($participant_data->category == 'Silver' ||$participant_data->category == 'Gold' ||$participant_data->category == 'Platinum' || $participant_data->category == 'Exhibitor')){?>
                                                            <?php if($participant_data->payment_state=='Confirmed'){?>
                                                            <li><a class="menu" href="<?=DN?>/receipt-single/<?=$participant_data->token?>" target="invoice_tab"><i class="fa fa-print icon"></i> Print Invoice</a></li>
                                                            <?php }?>
