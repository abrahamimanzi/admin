              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='Delegate-request' || `category`='Delegate request' || `category`='Government-request' || `category`='Government request' AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        $participantApprovalNumber = $participantTable ->count();
                    ?>
                      <h3 class="box-title">Request list <span style='color:#01aef0'><?=$participantApprovalNumber?></span></h3>
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
                                $participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `category`='Delegate-request' || `category`='Delegate request' || `category`='Government-request' || `category`='Government request' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                              ...
                              $participantTable = new Participant();
                              $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='Delegate-request' || `category`='Delegate request' || `category`='Government-request' || `category`='Government request' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
